...
def generate_simple_stats_by_covariates(model, covariates, covariates_df, feature_cols, config, 
                                       num_iterations=1000):
    """
    Generate simple statistics using the total variance formula:
    sigma squared = (1/K)sum(sigma squared k) + (1/K)sum((mu k - mu bar) squared)
    
    Where mu k and sigma squared k come from the model's output distribution parameters
    for each latent sample k.
    """
    ...
    try:
        ...
        for idx, covariate in enumerate(covariates_tensor):
            ...
            # Calculate overall mean: mu bar = (1/K)sum(mu k)
            overall_mean = np.mean(mu_k_array, axis=0)
            
            # Within-group variance: (1/K)sum(sigma squared k)
            within_variance = np.mean(sigma_k_array, axis=0)
            
            # Between-group variance: (1/K)sum((mu k - mu bar) squared)
...
def generate_simple_stats_from_encoded(model, features, covariates, covariates_raw, feature_cols, config, 
                                       num_iterations=1000):
    """
    Generate simple statistics from encoded features using the total variance formula:
    sigma squared = (1/K)sum(sigma squared k) + (1/K)sum((mu k - mu bar) squared)
...
def generate_bootstrap_stats_by_covariates(model, covariates, covariates_df, feature_cols, config, 
    ...
    """
    Generate bootstrap statistics using the total variance formula:
    sigma squared = (1/K)sum(sigma squared k) + (1/K)sum((mu k - mu bar) squared)
    
    For bootstrap version, we generate samples from the output distribution and then
    bootstrap resample to get multiple mu_k and sigma_k estimates.
    """
    device = torch.device("cuda" if config['device']['gpu'] and torch.cuda.is_available() else "cpu")
    model = model.to(device)
    model.eval()
    ...
    try:
        ...
        results = {
            ...
            'ci_total_variances_lower.csv': pd.concat([
                covariates_df,
                pd.DataFrame([ci[0] for ci in ci_total_variances_array], columns=feature_cols)
            ], axis=1),
            'ci_total_variances_upper.csv': pd.concat([
                covariates_df,
                pd.DataFrame([ci[1] for ci in ci_total_variances_array], columns=feature_cols)
            ], axis=1)
        }
...
def generate_bootstrap_stats_from_encoded(model, features, covariates, covariates_raw, feature_cols, config, 
                                        num_samples=1000, num_bootstraps=1000, confidence_level=0.95):
    """
    Generate bootstrap statistics from encoded features using the total variance formula:
    sigma squared = (1/K)sum(sigma squared k) + (1/K)sum((mu k - mu bar) squared)
    
    For encoded features, we first encode to latent space, then generate samples
    from the latent distribution and bootstrap resample.
    """
    device = torch.device("cuda" if config['device']['gpu'] and torch.cuda.is_available() else "cpu")
    model = model.to(device)
    model.eval()
...
def compute_feature_importance(bootstrap_results, covariates_df, feature_cols, config, use_bootstrap=True):
    """
    Compute feature importance based on results.
    
    Args:
        bootstrap_results: Dictionary containing DataFrames with results
        covariates_df: DataFrame containing original covariates
        feature_cols: List of feature column names
        config: Configuration dictionary
        use_bootstrap: Whether bootstrap analysis was used
    """
    try:
        # Select appropriate means DataFrame based on whether bootstrapping was used
        if use_bootstrap:
            means_key = 'bootstrapped_means.csv'
        else:
            means_key = 'means.csv'
            
        means_df = bootstrap_results[means_key]
...
def generate_summary_statistics(bootstrap_results, feature_cols, config, use_bootstrap=True):
    """Generate summary statistics from results."""
    try:
        # Select appropriate DataFrames based on whether bootstrapping was used
        if use_bootstrap:
            means_key = 'bootstrapped_means.csv'
            variances_key = 'bootstrapped_variances.csv'
        else:
            means_key = 'means.csv'
            variances_key = 'variances.csv'
            
        means_df = bootstrap_results[means_key]
        means_features = means_df[feature_cols]
        variances_df = bootstrap_results[variances_key]
